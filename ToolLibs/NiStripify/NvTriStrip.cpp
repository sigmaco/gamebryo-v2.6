...
//
bool SameTriangle(unsigned int firstTri0, unsigned int firstTri1, unsigned int firstTri2, 
                  unsigned int secondTri0, unsigned int secondTri1, unsigned int secondTri2)
{
    bool isSame = false;
...
bool TestTriangle(const unsigned int v0, const unsigned int v1, const unsigned int v2, const std::vector<NvFaceInfo>* in_bins, const int NUMBINS)
{
    //hash this triangle
    bool isLegit = false;
    int ctr = v0 % NUMBINS;
...
bool GenerateStrips(const unsigned int* in_indices, const unsigned int in_numIndices,
                    PrimitiveGroup** primGroups, unsigned int* numGroups, bool validateEnabled)
{
    ...
    {
        tempIndices[i] = (unsigned short)in_indices[i];
    ...
    if(bListsOnly)
    {
        ...
        *numGroups = 1;
        ...
        primGroupArray[0].numIndices = (unsigned int)numIndices;
    ...
    else
    {
        ...
        *numGroups = numSeparateStrips; //for the strips
        ...
        for(unsigned int stripCtr = 0; stripCtr < numSeparateStrips; stripCtr++)
        {
            ...
            primGroupArray[stripCtr].numIndices = (unsigned int)stripLength;
            
            int indexCtr = 0;
            for(int i = startingLoc; i < (int)(stripLength + startingLoc); i++)
                primGroupArray[stripCtr].indices[indexCtr++] = stripIndices[i];

            //we add 1 to account for the -1 separating strips
            //this doesn't break the stitched case since we'll exit the loop
            startingLoc += (int)stripLength + 1; 
        }
        ...
        for (unsigned int i = 0; i < *numGroups; ++i)
        {
            switch ((*primGroups)[i].type)
            {
                case PT_LIST:
                {
                    ...
                    {
                        unsigned int v0 = (*primGroups)[i].indices[j];
                        unsigned int v1 = (*primGroups)[i].indices[j + 1];
                        unsigned int v2 = (*primGroups)[i].indices[j + 2];
                ...
                case PT_STRIP:
                {
                    ...
                    {
                        unsigned int v0 = (*primGroups)[i].indices[j - 2];
                        unsigned int v1 = (*primGroups)[i].indices[j - 1];
                        unsigned int v2 = (*primGroups)[i].indices[j];
