...
//sckime 10/11/07 modified the cache inefficiency. This should
//be factored into the cache size, not a hidden magic number
//#define CACHE_INEFFICIENCY 6
#define CACHE_INEFFICIENCY 0
                                     ...
                                     const unsigned int maxIndex)
{
    ...
    unsigned int numIndices = (NiUInt32)indices.size();
    ...
    {    
        ...
        else 
        {
            ...
            {
                NiOutputDebugString("BuildStripifyInfo: > 2 triangles on an "
...
inline int NvStripifier::GetNextIndex(const WordVec &indices, NvFaceInfo *face){
    ...
    if (fv0 != v0 && fv0 != v1){
        if ((fv1 != v0 && fv1 != v1) || (fv2 != v0 && fv2 != v1)){
            NiOutputDebugString("GetNextIndex: Triangle doesn't have all of "
                "its vertices\n");
            NiOutputDebugString("GetNextIndex: Duplicate triangle probably "
                "got us derailed\n");
        }
        return fv0;
    }
...
void NvStripInfo::Build(NvEdgeInfoVec &edgeInfos, NvFaceInfoVec &)
{
    ...
    scratchIndices.push_back((const WORD)v0);
    scratchIndices.push_back((const WORD)v1);
    int v2 = NvStripifier::GetNextIndex(scratchIndices, m_startInfo.m_startFace);
    scratchIndices.push_back((const WORD)v2);
    ...
    while (nextFace != NULL && !IsMarked(nextFace))
    {
        ...
        if( (nextNextFace == NULL) || (IsMarked(nextNextFace)) )
        {
            ...
            if( ((testNextFace != NULL) && !IsMarked(testNextFace)) )
            {
                ...
                scratchIndices.push_back((const WORD)nv0);
                testnv0 = nv0;
        ...
        scratchIndices.push_back((const WORD)testnv1);
...
void NvStripInfo::Combine(const NvFaceInfoVec &forward, const NvFaceInfoVec &backward){
    ...
    for (int i = (int)(numFaces - 1); i >= 0; i--)
...
{
    return ( (face.m_v0 == (int)index) || 
        (face.m_v1 == (int)index) || (face.m_v2 == (int)index) );
}
                                ...
                                const bool bRestart, const unsigned int restartVal)
{
    ...
    for (int i = 0; i < (int)nStripCount; i++)
    {
        ...
        for (int j = 1; j < (int)nStripFaceCount; j++)
        {
                                            ...
                                            NvEdgeInfoVec& edgeInfos, NvFaceInfoVec& outFaceList)
{
    ...
    for(unsigned int i = 0; i < allStrips.size(); i++)
    {
        ...
        if(actualStripSize /*allStrips[i]->m_faces.size()*/ > threshold)
        {
            ...
            unsigned int numTimes    = actualStripSize /*allStrips[i]->m_faces.size()*/ / threshold;
            unsigned int numLeftover = actualStripSize /*allStrips[i]->m_faces.size()*/ % threshold;
            ...
            for(; j < numTimes; j++)
            {
                ...
                unsigned int faceCtr = j*threshold + degenerateCount;
            ...
            unsigned int leftOff = j * threshold + degenerateCount;
...
void NvStripifier::FindAllStrips(NvStripInfoVec &allStrips,
    ...
    while (!done)
    {
        ...
        // far we get
        //
        unsigned int numExperiments = experimentIndex;
