 ...
 ******************************************************************************
 $Header: //depot/3darg/Tools/NormalMapper/NmFileIO.cpp#23 $
 ******************************************************************************
...
NmWriteTriangles (FILE* fp, int numTris, NmRawTriangle* tris)
{
   ...
   NmHeader header;
   NiStrncpy (header.hdr, 4, NMF_HEADER_TAG, 4);
   ...
   NiStrncpy (header.hdr, 4, NMF_TRIANGLE_TAG, 4);
...
NmComputeTangents (int numTris, NmRawTriangle* tris, NmRawTangentSpace** tan)
{
   ...
   for (int i = 0; i < numTris; i++)
   {
      for (int j = 0; j < 3; j++)
      {
         (*tan)[i].tangent[j].v1.x = (float)(tanD[i].tangent[j].v1.x);
         (*tan)[i].tangent[j].v1.y = (float)(tanD[i].tangent[j].v1.y);
         (*tan)[i].tangent[j].v1.z = (float)(tanD[i].tangent[j].v1.z);
         (*tan)[i].binormal[j].v1.x = (float)(tanD[i].binormal[j].v1.x);
         (*tan)[i].binormal[j].v1.y = (float)(tanD[i].binormal[j].v1.y);
         (*tan)[i].binormal[j].v1.z = (float)(tanD[i].binormal[j].v1.z);
      }
   }
   delete [] tanD;
   return true;
...
NmCompareD (NmTangentPointD* v0, NmTangentPointD* v1)
{
   ...
   // Positions.
   // Emergent Change 2/14/05
   // Fix for-loop scope compliance
   int i;
   for (i = 0; i < 3; i++)
   // END Emergent
   {
                        ...
                        double tan[3], double norm[3], double binorm[3])
{
   ...
   norm[0] = pgon->norm[idx].v1.x;
   norm[1] = pgon->norm[idx].v1.y;
   norm[2] = pgon->norm[idx].v1.z;

   // Make sure we have valid data.
   if (pgon->texCoord[0].v1.u == 0.0 && 
       pgon->texCoord[1].v1.u == 0.0 && 
       pgon->texCoord[2].v1.u == 0.0)
   {
      return;
   }

   DcTangentSpace(pgon->vert[0].v2.v, pgon->vert[1].v2.v, pgon->vert[2].v2.v, 
                  pgon->texCoord[0].v2.uv, pgon->texCoord[1].v2.uv,
                  pgon->texCoord[2].v2.uv, norm, tan, binorm);
...
{
   point->vertex[0] = tri->vert[v].v1.x;
   point->vertex[1] = tri->vert[v].v1.y;
   point->vertex[2] = tri->vert[v].v1.z;
   point->normal[0] = tri->norm[v].v1.x;
   point->normal[1] = tri->norm[v].v1.y;
   point->normal[2] = tri->norm[v].v1.z;
   point->uv[0] = tri->texCoord[v].v1.u;
   point->uv[1] = tri->texCoord[v].v1.v;
   double norm[3];
                     ...
                     int v, NmTangentPointD* point)
{
   ...
   point->uv[0] = tri->texCoord[v].v1.u;
   point->uv[1] = tri->texCoord[v].v1.v;
   point->tangent[0] = tan->tangent[v].v1.x;
   point->tangent[1] = tan->tangent[v].v1.y;
   point->tangent[2] = tan->tangent[v].v1.z;
   point->binormal[0] = tan->binormal[v].v1.x;
   point->binormal[1] = tan->binormal[v].v1.y;
   point->binormal[2] = tan->binormal[v].v1.z;
   point->count = 1;
}*/
...
NmComputeTangentsD (int numTris, NmRawTriangle* tris, NmRawTangentSpaceD** tan)
{
   ...
   for (i = 0; i < numTris; i++)
   {
      for (int j = 0; j < 3; j++)
      {
         (*tan)[i].tangent[j].v1.x = point[index[i].idx[j]].tangent[0];
         (*tan)[i].tangent[j].v1.y = point[index[i].idx[j]].tangent[1];
         (*tan)[i].tangent[j].v1.z = point[index[i].idx[j]].tangent[2];
         (*tan)[i].binormal[j].v1.x = point[index[i].idx[j]].binormal[0];
         (*tan)[i].binormal[j].v1.y = point[index[i].idx[j]].binormal[1];
         (*tan)[i].binormal[j].v1.z = point[index[i].idx[j]].binormal[2];
      }
   }
