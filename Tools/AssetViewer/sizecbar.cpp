...
//
/////////////////////////////////////////////////////////////////////////
// Copyright (C) 1998-2007 by Cristi Posea. All rights reserved.
//
...
void CSizingControlBar::OnNcPaint()
{
    ...
    mdc.FillRect(rcDraw, CBrush::FromHandle(
        (HBRUSH) UlongToPtr(GetClassLong(m_hWnd, GCL_HBRBACKGROUND))));

    if (m_dwSCBStyle & SCBS_SHOWEDGES)
    {
        CRect rcEdge; // paint the sizing edges
        for (int i = 0; i < 4; i++)
            if (GetEdgeRect(rcBar, (UINT)GetEdgeHTCode(i), rcEdge))
...
void CSizingControlBar::OnTrackUpdateSize(CPoint& point)
{
    ...
    else
    {
        ...
        int nLimit = (int)(bBefore ? -1 : arrSCBars.GetSize());
...
#if defined(_MSC_VER) && _MSC_VER >= 1400
LRESULT CSizingControlBar::GetEdgeHTCode(int nEdge)
#else //#if defined(_MSC_VER) && _MSC_VER >= 1400
...
void CSizingControlBar::LoadState(LPCTSTR lpszProfileName)
{
    ...
    for (int i = 0; i < state.m_arrBarInfo.GetSize(); i++)
    {
        ...
        for (int j = 0; j < pInfo->m_arrBarID.GetSize(); j++)
            if ((DWORD) PtrToUlong(pInfo->m_arrBarID[j]) == nID)
            {
