...
int CRollupCtrl::_InsertPage(LPCTSTR caption, CDialog* pwndTemplate, int idx, BOOL bAutoDestroyTpl)
{
    ...
    pi->pOldDlgProc     = (WNDPROC)LongToPtr(::GetWindowLong(pwndTemplate->m_hWnd, DWL_DLGPROC));
    pi->pOldButProc     = (WNDPROC)LongToPtr(::GetWindowLong(but->m_hWnd, GWL_WNDPROC));
    ...
    ::SetWindowLong(pwndTemplate->m_hWnd, GWL_USERDATA, PtrToLong(m_PageList[newidx]));
    ::SetWindowLong(pwndTemplate->m_hWnd, DWL_USER,     PtrToLong(this));

    //Set But Window data
    ::SetWindowLong(but->m_hWnd, GWL_USERDATA,  PtrToLong(m_PageList[newidx]));

    //SubClass Template window proc
    ::SetWindowLong(pwndTemplate->m_hWnd, DWL_DLGPROC, PtrToLong(CRollupCtrl::DlgWindowProc));

    //SubClass Button window proc
    ::SetWindowLong(but->m_hWnd, GWL_WNDPROC, PtrToLong(CRollupCtrl::ButWindowProc));
...
{
    RC_PAGEINFO* pi     = (RC_PAGEINFO*)LongToPtr(GetWindowLong(hWnd, GWL_USERDATA));
    CRollupCtrl* _this  = (CRollupCtrl*)LongToPtr(GetWindowLong(hWnd, DWL_USER));
